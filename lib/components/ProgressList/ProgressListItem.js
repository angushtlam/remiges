...
const pulsing = keyframes`
  0%, 100% {background: ${colors.interactable};}
  50% {background: ${colors.blank};}
`

const Slot = styled.div`
  ${props => {
    if (props.isCurrent) {
      return `animation: 3s ${pulsing} ease-in-out infinite;`
    } else if (props.isFilled) {
      return `background: ${colors.interactable}`
    } else {
      return `background: ${colors.blank}`
    }
  }};
