...
class Column extends React.Component {
  ...
  render() {
    const {className, children, fixed, width, ...otherProps} = this.props

    if (!fixed) {
      return (
        <Division className={className} width={width} {...otherProps}>
          {children}
        </Division>
      )
    }
...
export default constructWithTheme(Paddable(Column))
